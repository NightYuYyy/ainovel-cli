...
const (
	modeLibrary appMode = iota // 图书馆视图 - 任务列表
	modeNew                    // 等待用户输入小说需求
...
// NewLibraryModel 创建图书馆模式 TUI Model。
func NewLibraryModel(lib *library.Manager, version string) Model {
	ta := textarea.New()
	ta.Placeholder = "输入新书名和需求，如：写一本东方玄幻长篇..."
	ta.CharLimit = 2000
	ta.SetHeight(1)
	ta.MaxHeight = 6
	ta.ShowLineNumbers = false
	...
	return Model{
		libManager:   lib,
		libraryState: ls,
		runtime:      nil, // 选中任务后才创建
		askBridge:    nil, // 选中任务后才创建
		version:      strings.TrimSpace(version),
		autoScroll:   true,
		streamScroll: true,
		mode:         modeLibrary,
...
func (m Model) View() string {
	...
	var body string
	if m.mode == modeLibrary {
		body = m.renderLibrary(m.width, bodyH)
